...
func UnknownSigner() tests.SpecTest {
	...
	msgF := func(obj *types.ConsensusData, id []byte) *qbft.SignedMessage {
		...
		if len(obj.PreConsensusJustifications) > 0 {
			obj.PreConsensusJustifications[0].Signer = 5
	...
	expectedErr := "failed processing consensus message: invalid pre-consensus justification: failed to verify PartialSignature: unknown signer"

	return &tests.MultiMsgProcessingSpecTest{
		Name: "pre consensus unknown signer",
